...
const THEME_CHANGED_EVENT = 'carhistorial_theme_changed';
            ...
            {({ router }) => (
                <div className="space-y-6">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 mt-1">Tema, usuario y seguridad de tu cuenta.</p>
                    </div>
                    ...
                    <section className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-6">
                        ...
                        <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-5 space-y-6">
                            <div>
                                <h3 className="font-black mb-3">Tema</h3>
                                <div className="inline-flex rounded-lg bg-slate-100 dark:bg-slate-800 p-1">
                                    {['light', 'dark'].map((option) => (
                                        <button key={option} onClick={() => changeTheme(option)} className={`rounded-md px-4 py-2 text-sm font-bold ${theme === option ? 'bg-white dark:bg-slate-900 text-primary shadow-sm' : 'text-slate-500'}`}>{option === 'light' ? 'Claro' : 'Oscuro'}</button>
                                    ))}
                                </div>
                            </div>

                            <ProfileForm key={`${session.nombre || session.name || ''}-${session.email || ''}`} session={session} onSubmit={updateSession} />
                            <PasswordForm onSubmit={(password) => updateSession({ ...session, passwordUpdatedAt: new Date().toISOString(), passwordHint: password.slice(0, 1) })} />

                            <div className="border-t border-slate-100 dark:border-slate-800 pt-5">
                                <button onClick={() => { clearSession(); router.push(`/${lang}/login`); }} className="rounded-lg bg-red-50 px-5 py-2 text-sm font-black text-red-600">Cerrar sesion</button>
                            </div>
                        </div>
                    </section>
                </div>
...
function ProfileForm({ session, onSubmit }) {
    ...
    return (
        <form className="grid grid-cols-1 md:grid-cols-2 gap-3" onSubmit={(event) => { event.preventDefault(); onSubmit({ ...session, nombre: form.nombre, name: form.nombre, email: form.email }); }}>
            <h3 className="md:col-span-2 font-black">Gestion del usuario</h3>
            <Field label="Nombre"><input className={inputClass} value={form.nombre} onChange={(e) => setForm({ ...form, nombre: e.target.value })} /></Field>
            <Field label="Correo"><input type="email" className={inputClass} value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} /></Field>
            <div className="md:col-span-2"><button className="rounded-lg bg-primary px-5 py-2 text-sm font-black text-white">Guardar perfil</button></div>
        </form>
    );
}
...
function PasswordForm({ onSubmit }) {
    ...
    return (
        <form className="grid grid-cols-1 md:grid-cols-3 gap-3" onSubmit={(event) => {
            ...
            if (form.next.length < 6) return setError('La nueva contrasena debe tener al menos 6 caracteres.');
            if (form.next !== form.repeat) return setError('Las contrasenas no coinciden.');
            setError(null);
            onSubmit(form.next);
            setForm({ current: '', next: '', repeat: '' });
        }}>
            <h3 className="md:col-span-3 font-black">Cambiar contrasena</h3>
            {error && <div className="md:col-span-3 rounded-lg bg-red-50 p-3 text-sm font-bold text-red-600">{error}</div>}
            <Field label="Actual"><input type="password" className={inputClass} value={form.current} onChange={(e) => setForm({ ...form, current: e.target.value })} /></Field>
            <Field label="Nueva"><input type="password" className={inputClass} value={form.next} onChange={(e) => setForm({ ...form, next: e.target.value })} /></Field>
            <Field label="Repetir"><input type="password" className={inputClass} value={form.repeat} onChange={(e) => setForm({ ...form, repeat: e.target.value })} /></Field>
            <div className="md:col-span-3"><button className="rounded-lg bg-slate-900 dark:bg-white px-5 py-2 text-sm font-black text-white dark:text-slate-900">Actualizar contrasena</button></div>
        </form>
    );
}
...
function subscribeToTheme(listener) {
    ...
    const handleStorage = (event) => {
        if (!event.key || event.key === 'theme') listener();
    };
