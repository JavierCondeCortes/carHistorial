                        ...
                        <div className="min-h-72 rounded-xl overflow-hidden relative bg-slate-900">
                            ...
                            <div className="absolute left-6 right-6 bottom-6">
                                ...
                                <div className="flex flex-wrap gap-2 mt-4">
                                    ...
                                    <Badge icon="speed" label={`${Number(vehicle?.km_recorridos || 0).toLocaleString()} km`} />
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                        <ResourcePanel title={t('vehicleDetail.sections.liquids', 'Liquidos')} icon="opacity" items={resources.liquids} loading={resources.loading || loadingVehicles} empty={t('vehicleDetail.empty.liquids', 'Sin liquidos registrados')}>
                            {(item) => {
                                ...
                                return <ResourceRow title={item.nombre} meta={`${item.tipo || 'N/A'} - ${item.km_para_cambio || 0} km - ${formatDate(localData?.fecha_cambio_actual)}`} attachment={attachment} onEdit={() => setActiveForm({ type: 'liquid', item })} onDelete={() => submitAction(() => resources.removeLiquid(item.id))} />;
                            }}
                        </ResourcePanel>
                        ...
                        <ResourcePanel title={t('vehicleDetail.sections.maintenance', 'Mantenimientos')} icon="build" items={resources.maintenances} loading={resources.loading} empty={t('vehicleDetail.empty.maintenance', 'Sin mantenimientos registrados')}>
                            {(item) => <ResourceRow title={item.componente_cambiado} meta={`${Number(item.km_cambiado || 0).toLocaleString()} km - ${formatDate(item.fecha_cambio)} - ${formatCurrency(item.costo)}`} attachment={getResourceAttachment('maintenance', item)} onEdit={() => setActiveForm({ type: 'maintenance', item })} onDelete={() => submitAction(() => resources.removeMaintenance(item.id))} />}
                        </ResourcePanel>
                    </div>
